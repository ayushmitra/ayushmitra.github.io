...
if (!function_exists('see_my_cv_system_fonts_list')) {
	function see_my_cv_system_fonts_list() {
		$fonts = array( 
...
if (!function_exists('see_my_cv_get_google_fonts_list')) {
	function see_my_cv_get_google_fonts_list() {
		$fonts = array( 
			'' => esc_html__('None', 'see-my-cv'), 
		...
		return apply_filters('see_my_cv_google_fonts_list_filter', $fonts);
	}
}
...
if (!function_exists('see_my_cv_text_decoration_list')) {
	function see_my_cv_text_decoration_list() {
		$list = array( 
			'none' => esc_html__('none', 'see-my-cv'), 
			'underline' => esc_html__('underline', 'see-my-cv'), 
			'overline' => esc_html__('overline', 'see-my-cv'), 
			'line-through' => esc_html__('line-through', 'see-my-cv'), 
		);
...
if (!function_exists('see_my_cv_custom_color_schemes_list')) {
	function see_my_cv_custom_color_schemes_list() {
		$list = array( 
			'first' => esc_html__('Custom 1', 'see-my-cv'), 
			'second' => esc_html__('Custom 2', 'see-my-cv'), 
			'third' => esc_html__('Custom 3', 'see-my-cv') 
		);
		
		
		return apply_filters('see_my_cv_custom_color_schemes_list_filter', $list);
	}
}
...
if (!function_exists('see_my_cv_locate_template')) {
	function see_my_cv_locate_template($template_names, $require_once = true, $load = true) {
		$located = '';
...
define('CMSMASTERS_MEGA_MENU', false);
...
define('CMSMASTERS_MAILPOET', class_exists('WYSIJA') ? false : false);

// Theme Style Compatibility
define('CMSMASTERS_THEME_STYLE_COMPATIBILITY', false);

// Theme Style
define('CMSMASTERS_THEME_STYLE', (CMSMASTERS_THEME_STYLE_COMPATIBILITY && get_option('cmsmasters_see_my_cv_theme_style') ? get_option('cmsmasters_see_my_cv_theme_style') : ''));
...
if (!function_exists('see_my_cv_all_theme_styles')) {
	function see_my_cv_all_theme_styles() {
		$out = array( 
			'Default|', 
			'Theme Style 1|1', 
			'Theme Style 2|2' 
...
if (!function_exists('see_my_cv_all_color_schemes_list')) {
	function see_my_cv_all_color_schemes_list() {
		$list = array( 
			'default' => 		esc_html__('Default', 'see-my-cv'), 
			'header' => 		esc_html__('Header', 'see-my-cv'), 
			'navigation' => 	esc_html__('Navigation', 'see-my-cv'), 
			'header_top' => 	esc_html__('Header Top', 'see-my-cv'), 
			'footer' => 		esc_html__('Footer', 'see-my-cv') 
		);
		
		
		$out = array_merge($list, see_my_cv_custom_color_schemes_list());
...
if (!function_exists('see_my_cv_load_theme_textdomain')) {
	function see_my_cv_load_theme_textdomain() {
		load_theme_textdomain('see-my-cv', get_template_directory() . '/theme-framework/languages');
	}
}

// Load Theme Local File Action
if (!has_action('after_setup_theme', 'see_my_cv_load_theme_textdomain')) {
	add_action('after_setup_theme', 'see_my_cv_load_theme_textdomain');
}



// Framework Activation & Data Import
if (!function_exists('see_my_cv_theme_activation')) {
	function see_my_cv_theme_activation() {
		if (get_option('cmsmasters_active_theme') != 'see-my-cv') {
			add_option('cmsmasters_active_theme', 'see-my-cv', '', 'yes');
			
			
			see_my_cv_add_global_options();
			
			
			see_my_cv_add_global_icons();
...
add_action('after_switch_theme', 'see_my_cv_theme_activation');



// Framework Deactivation
if (!function_exists('see_my_cv_theme_deactivation')) {
	function see_my_cv_theme_deactivation() {
		delete_option('cmsmasters_active_theme');
	}
}

add_action('switch_theme', 'see_my_cv_theme_deactivation');



// Plugin Activation Regenerate Styles
if (!function_exists('see_my_cv_plugin_activation')) {
	function see_my_cv_plugin_activation() {
		update_option('cmsmasters_plugin_activation', 'true');
	}
}

add_action('activated_plugin', 'see_my_cv_plugin_activation');


if (!function_exists('see_my_cv_plugin_activation_regenerate')) {
	function see_my_cv_plugin_activation_regenerate() {
...
add_action('init', 'see_my_cv_plugin_activation_regenerate');
